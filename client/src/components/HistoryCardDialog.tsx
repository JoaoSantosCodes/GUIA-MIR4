...
export default function HistoryCardDialog({ open, onOpenChange, entries, userName, goldBadges }: Props) {
  const [style, setStyle] = useState<CardStyle>(DEFAULT_CARD_STYLE);
  const [exporting, setExporting] = useState(false);

  const runExport = async (drawTo?: HTMLCanvasElement) => {
    await exportHistoryCard({ entries, userName, goldBadges, style, drawTo });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-amber-300">Card do histórico de conquistas</DialogTitle>
          <DialogDescription>
            Personalize e exporte um card com todas as suas medalhas — Codex e Dicas de Ouro — para postar nas redes sociais.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-slate-500">
            <p>🏆 {entries.length} medalha{entries.length !== 1 ? "s" : ""} no card</p>
            {goldBadges > 0 && <p>★ {goldBadges} Dica{goldBadges !== 1 ? "s" : ""} de Ouro</p>}
          </div>
        </div>
